...
export function TornadoRadar() {
  const { data: alerts = [], isLoading, isError } = useTornadoAlerts()
  ...
  return (
    <PanelCard title="Tornado Doppler Radar" subtitle="IEM NEXRAD + NWS Alerts" status={status} badge={alerts.length ? `${alerts.length} ACTIVE` : 'CLEAR'}>
      <div className="flex flex-col h-full gap-2">
        <AlertBanner alerts={alerts} />
        <div className="flex-1 min-h-[220px] rounded-lg overflow-hidden">
